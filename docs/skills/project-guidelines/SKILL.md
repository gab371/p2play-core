...
- **Duplicating `SoundToggle` or shadcn primitives per game**: Prefer `Button` / `Badge` / `SoundToggle` from `p2play-core/ui` (inject local `soundManager`). Apps must scan `p2play-core` in Tailwind `content` and provide CSS variables. Hub may keep its own shadcn set under `src/components/ui/` when it needs hub-only surfaces.
- **Reimplementing “Copier le lien” text buttons**: Use `CopyRoomLinkButton` (icon-only) next to the room code; in-game headers use `RoomCodeBadge` (code + copy icon in one pill). Both copy the shareable room URL via `copyRoomUrlToClipboard`.
- **Reimplementing text chat / activity journal UI**: Use `TextChatPanel` / `JournalPanel` from `p2play-core/chat`. Pass `scrollbarAccent` (and optional `typeClassNames`) so journal event colors match the game theme.
...
- **Dual React / Invalid hook call**: Always set Vite `resolve.dedupe: ["react", "react-dom"]` (Hub and games). Lib mode also aliases react to the app's `node_modules`. Nested `react` inside a `file:../p2play-core` install causes black screens.
...
- **Auto-starting an embedded game that has pre-game configuration**: In embedded mode, populate players from `peerManager.lobbyPlayers` but stay in `LOBBY` phase — never call `engine.startGame()` automatically if the game exposes a pre-game config lobby. See [rules/hub-integration.md §9](./rules/hub-integration.md).
