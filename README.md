...
- 💾 **Session (`p2play-core/session`)**: room reconnect helpers (`saveSession` / `loadSession`) + durable profile (`saveProfile` / `loadProfile`, key `p2play:profile`) for multi-day pseudo/avatar; `sessionToken` verified on reconnect.
